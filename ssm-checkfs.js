...
try {
    ...
    var volSize = Number(input.CurrentSize);

    var cmd = "BLOCKCOUNT=$(sudo dumpe2fs -h /dev/"+device+" | grep 'Block count' | awk '{print $3}') && BLOCKSIZE=$(sudo dumpe2fs -h /dev/"+device+" | grep 'Block size' | awk '{print $3}') && echo \"$(($BLOCKCOUNT * $BLOCKSIZE))\"";

    var result = runCommand(input.AWSAccessKey, input.AWSSecretKey, input.AWSRegion, input.InstanceId, cmd);
    if (!result[0]) {
        throw new Error(result[1]);
    }

    // AWS has the volume size in GiB ... we've just retrieved the number of bytes, so convert ...
    var fsSize = Math.ceil(Number(result[1]) / 1073741824);
    // There may be a rounding error, so allow it to be within 1 of the current size
    console.log((volSize-1)+" <= "+fsSize+" <= "+volSize);
    if (volSize-1 <= fsSize && fsSize <= volSize) {
        output.CommandSuccess = true;
    } else {
        output.CommandSuccess = false;
        output.ErrorMessage = "It looks like the volume has been expanded but the filing system not resized. Need to handle this manually.";
    }
} catch (error) {
